...
fn main() -> Result<()> {
    // В init_logging використовується try_init(), тому паніка тут малоймовірна.
...
mod tests {
    ...
    fn store_save_and_overwrite() -> Result<()> {
        ...
        // Перевіряємо, чи коректно оновлені дані
        let loaded_store = Store::load(path)?;
        assert_eq!(
            loaded_store.snippets.len(),
            1,
            "Має бути 1 сніпет після перезапису"
        );
        assert!(
            loaded_store.snippets.contains_key("new_key"),
            "Новий ключ має бути присутній"
        );
        assert!(
            !loaded_store.snippets.contains_key("old_key"),
            "Старий ключ має бути видалений"
        );
